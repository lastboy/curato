...
export async function setup(argv) {
        ...
        if (process.platform !== 'darwin') {
            ...
            if (dryRun) {
                ...
                try {
                    ...
                    if (!result.loaded)
                        warn('  launchctl load failed — reboot or load manually');
                }
                catch (err) {
    ...
    if (cfg.marketplaces && Object.keys(cfg.marketplaces).length > 0) {
        ...
        for (const [name, entry] of Object.entries(cfg.marketplaces)) {
            if (entry.enabled === false) {
                dim(`  skip (disabled): ${name}`);
                continue;
            }
            hasWork = true;
            if (dryRun) {
                info(`  would add marketplace: ${name} (${entry.source})`);
                continue;
            }
    ...
    if (cfg.mcpServers && Object.keys(cfg.mcpServers).length > 0) {
        ...
        for (const [name, entry] of Object.entries(cfg.mcpServers)) {
            ...
            if (entry.scope === 'project') {
                ...
                if (name in servers) {
                    dim(`  already registered (project): ${name}`);
                    continue;
                }
                ...
                const vsServers = vsSettings['mcpServers'] ?? {};
                ...
                if (inVs && inCli) {
                    dim(`  already registered (user): ${name}`);
                    continue;
                }
    ...
    // ── CLAUDE.md ────────────────────────────────────────────────────────────────
    for (const [scope, scopeDir] of [['project', cwd], ['user', claudeDir]]) {
