 ...
 */
export function assertSafeName(value: string, kind: 'plugin' | 'skill' | 'marketplace'): void {
  ...
  if (!NAME_RE.test(value)) {
    throw new Error(
      `Invalid ${kind} name "${value}": only letters, digits, dot, underscore, hyphen allowed (must start with letter or digit)`
    );
  }
}
 ...
 */
export function assertPathUnderHome(filePath: string, label: string): string {
