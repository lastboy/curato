...
import { applySkillFilter, reportSkillCosts, findPluginCachePath, compareVersions } from '../../patcher/skill-filter.js';
...
describe('compareVersions', () => {
  it('sorts 1.10.0 after 1.9.0', () => {
    const versions = ['1.10.0', '1.2.0', '1.9.0'].sort(compareVersions);
    assert.deepEqual(versions, ['1.2.0', '1.9.0', '1.10.0']);
  });

  it('returns highest version last for mixed patch levels', () => {
    const versions = ['2.0.1', '2.0.10', '2.0.2'].sort(compareVersions);
    assert.equal(versions[versions.length - 1], '2.0.10');
  });

  it('handles missing segments (1.0 vs 1.0.0)', () => {
    assert.equal(compareVersions('1.0', '1.0.0'), 0);
    assert.equal(compareVersions('1.0.1', '1.0'), 1);
  });

  it('falls back to lexical for non-numeric segments', () => {
    const versions = ['1.0.0-alpha', '1.0.0-beta'].sort(compareVersions);
    assert.deepEqual(versions, ['1.0.0-alpha', '1.0.0-beta']);
  });
