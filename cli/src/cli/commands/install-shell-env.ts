...
export async function installShellEnvCmd(argv: string[]) {
  ...
  let vars: string[] = (values.var as string[] | undefined) ?? [];

  // If --config is passed (even with no value), read vars from curato-setup.json.
  if ('config' in values) {
    const configPath = (values.config as string | undefined) ?? join(process.cwd(), 'curato-setup.json');
    try {
      const raw = JSON.parse(readFileSync(configPath, 'utf8')) as { shellEnv?: { vars?: string[] } };
  ...
  const sourceFile = values['source-file'] as string | undefined;
  ...
  try {
    ...
    if (result.loaded) ok('LaunchAgent loaded into launchd');
    else warn('Plist written, but launchctl load failed — try: launchctl load ' + result.plistPath);
