...
export async function teardown(argv: string[]) {
  ...
  if (cfg.plugins && cfg.plugins.length > 0) {
    ...
    for (const pluginEntry of cfg.plugins) {
      ...
      if (dryRun) { info(`  would uninstall: ${name}`); continue; }
      ...
      if (r.status === 0) ok(`  uninstalled: ${name}`);
      else warn(`  may not have been installed: ${name}`);
    }
  }
  ...
  if (cfg.mcpServers && Object.keys(cfg.mcpServers).length > 0) {
    ...
    for (const [name] of Object.entries(cfg.mcpServers)) {
      didWork = true;
      if (dryRun) { info(`  would remove: ${name}`); continue; }
  ...
  if (cfg.marketplaces && Object.keys(cfg.marketplaces).length > 0) {
    ...
    for (const [name] of Object.entries(cfg.marketplaces)) {
      didWork = true;
      if (dryRun) { info(`  would remove marketplace: ${name}`); continue; }
      ...
      if (r.status === 0) ok(`  removed marketplace: ${name}`);
      else warn(`  may not have been registered: ${name}`);
    }
  }
  ...
  if (cfg.shellEnv && cfg.shellEnv.vars.length > 0) {
    ...
    if (process.platform !== 'darwin') {
      ...
      if (dryRun) {
        ...
        if (result.existed) ok(`  removed LaunchAgent: ${result.plistPath}`);
        else dim(`  no LaunchAgent found (already removed)`);
        if (result.unsetVars.length > 0) dim(`  unset: ${result.unsetVars.join(', ')}`);
      }
    }
  }
