...
export async function registerMcp(argv: string[]) {
  ...
  for (const pair of (values.env ?? []) as string[]) {
    const idx = pair.indexOf('=');
    if (idx < 1) { fail(`Invalid --env format: "${pair}" (expected KEY=VALUE)`); process.exit(1); }
  ...
  if (scope === 'project') {
    ...
    if (existsSync(mcpJsonPath)) {
      try { existing = JSON.parse(readFileSync(mcpJsonPath, 'utf8')) as Record<string, unknown>; } catch { /* ignore */ }
    }
    const servers = (existing['mcpServers'] as Record<string, unknown>) ?? {};
    if (name in servers) { dim(`Already registered in .mcp.json: ${name}`); return; }
  ...
  if (existsSync(claudeJsonPath)) {
    try { cliJson = JSON.parse(readFileSync(claudeJsonPath, 'utf8')) as Record<string, unknown>; } catch { /* ignore */ }
  }
