...
export async function install(argv: string[]) {
  ...
  if (dryRun) {
    ...
    if (hasFilter) info(`dry-run: would apply skill filter  exclude=[${exclude}]  include=[${include}]`);
    return;
  }
  ...
  for (const s of disabled) ok(`  disabled: ${s.name}`);
  for (const s of kept)     info(`  kept:     ${s.name}`);
