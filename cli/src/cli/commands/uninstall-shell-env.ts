...
export async function uninstallShellEnvCmd(argv: string[]) {
  ...
  const keepVars = values['keep-vars'] as boolean;
  ...
  try {
    ...
    if (result.unloaded) ok('Unloaded from launchd');
    else warn('Unload may have failed — reboot or relogin to be sure');
