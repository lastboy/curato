...
import type { ProjectLayoutInfo } from '../types.js';
...
export function scanProjectLayout(cwd: string = process.cwd()): ProjectLayoutInfo {
  ...
  if (existsSync(skillsDir)) {
    try {
      ...
      for (const name of skillNames) {
        ...
        else if (existsSync(skillMdCap)) skillFiles.push(skillMdCap);
      }
    } catch {
      // ignore
    }
